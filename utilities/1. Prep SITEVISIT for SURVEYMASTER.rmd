...
sectors<-read.csv("C:/Users/Tye.Kindinger/Desktop/FISH TEAM/fish-paste/data/Sectors-Strata-Areas.csv", stringsAsFactors=FALSE)
...
sv.sec <- sv.ready %>% mutate(SEC_NAME = ifelse(ISLAND != "Guam" & REGION == "MARIAN", ISLAND, # use ISLAND spelling for non-Guam SEC_NAME
                                                ...
                                                ifelse(SEC_NAME == "GUA_TUMON_BAY", "GUA_TUMON", SEC_NAME)))) %>% distinct(SEC_NAME)

sv.ready %>% distinct(REGION)

SM %>% filter(OBS_YEAR > 2009 & OBS_YEAR < 2022 & REGION == "MARIAN") %>% filter(is.na(SEC_NAME)) # Note: NA is an Oceanography site (can ignore for Fish)

sm.final %>% filter(OBS_YEAR > 2009 & OBS_YEAR < 2022 & REGION == "MARIAN" & !is.na(SEC_NAME)) %>% distinct(SEC_NAME) %>% arrange(SEC_NAME)

sm.final %>% distinct(REGION)
```
...
df <- fish.sm %>% # 37,042 rows
  ...
  # clean up remaining
  mutate_at(vars(SECTOR, OTHER_TYPE, COMPLEXITY, HYBRID, OLD_SIZE_BINS), ~as.factor(.)) %>% 
  mutate_at(vars(HUMANPOPULATIONWITHIN20KM, SURVEY_LENGTH_M, SURVEY_WIDTH_M,  FISH_BLT_CORISID, FISH_SPC_CORISID, SPECIAL_PROJ_YN), ~as.integer(.)) %>%
  ...
  # REMOVE columns from SurveyMaster
  select(-SEC_NAME, -SPECIAL_PROJ_YN, -SPECIAL_PROJ_DESCRIPTION) %>%
  # ADD prior data
