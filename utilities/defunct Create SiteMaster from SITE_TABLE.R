...
SM_FIELDS<-c("SEC_NAME", "HUMANS20", "HUMANS200", "TYPE", "ANALYSIS_SCHEME", "ANALYSIS_YEAR", "TUT2012", "OTHER_AREA_GROUPING", "SPECIAL_PROJ_YN", "SPECIAL_PROJ_DESCRIPTION")
...
ccr.sv$METHOD<-"xx"
ccr.sv[ccr.sv$SITEVISITID %in% CCRsv,]$METHOD="CCR"
c<-subset(ccr.sv, ccr.sv$METHOD %in% "CCR")
s<-subset(ccr.sv, !ccr.sv$METHOD %in% "CCR")

c<-c[,names(c)[!names(c) %in% SM_FIELDS]]
c<-merge(c, s[,c("SITE", SM_FIELDS)], by="SITE", all.x=T)
dim(c)
cs<-rbind(c,s)
cs$METHOD<-NULL
