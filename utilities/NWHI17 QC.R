rm(list=ls())
# SET UP ------------------------------------------------------------------
setwd("/Users/ivor.williams/Documents/CRED/Fish Team/Cruises/NWHI17")
...
x<-read.csv("HA1704.csv")    #NB... 'REA FISH BASE FOR R' IS THE ACCESS QUERY THAT GENERATES OUTPUT TO MATCH THE DATA_COLS ABOVE
...
"HARD_CORAL", "MA",  "TA",  "CCA",  "SAND",  "OTHER",
"SPECIES", "TAXONNAME", "COUNT", "SIZE_", "OBS_TYPE", 
"FAMILY" , "COMMONFAMILYALL", "LMAX", "LW_A",  "LW_B",  "LENGTH_CONVERSION_FACTOR", "TROPHIC", "TROPHIC_MONREP", "TRAINING_YN")
head(x[,DATA_COLS])
x<-x[,DATA_COLS]
...
SURVEY_INFO<-c("OBS_YEAR", "REGION", "ISLAND", "SITE", "REEF_ZONE", "DEPTH_BIN", "LATITUDE", "LONGITUDE", "SITEVISITID", "METHOD")
survey_table<-Aggregate_InputTable(wd, SURVEY_INFO)
island_table<-Aggregate_InputTable(wd, c("REGION","ISLAND"))
SURVEY_SITE_DATA<-c("DEPTH", "HARD_CORAL", "MA", "CCA", "SAND", "OTHER")
...
divers<-levels(compdata$DIVER); divers

# COMPARISON ON TOTAL FISH BIOMASS
msd<-data.frame(with(compdata, tapply(TotFish, list(SITEVISITID, DIVER), mean))) ## mean biomass estimate per diver per site
...
toplot<-(melt(toplot)); summary(toplot)
names(toplot)<-c("diver_diff", "diver")

#set to MAX diff, so we can see it in the plot
AXIS_RANGE<-75
toplot[toplot$diver_diff< -AXIS_RANGE,]$diver_diff<- -AXIS_RANGE
toplot[toplot$diver_diff> AXIS_RANGE,]$diver_diff<- AXIS_RANGE
toplot$diver<-as.factor(toplot$diver)

bio <- ggplot(toplot, aes(factor(diver), diver_diff))
bio <- bio + 
  geom_boxplot() + 
  coord_flip() + 
  geom_jitter(colour = "red", size =1, alpha = 0.4) + 
  theme_bw() +
  scale_y_continuous(limits=c(-AXIS_RANGE,AXIS_RANGE))+
  labs(y = expression(paste("Difference in biomass estimate (g ", m^-2,") relative to buddy")), x = "Diver") + 
  scale_x_discrete(breaks = 1:length(levels(compdata$DIVER)), labels = divers)
...
msd<-data.frame(with(compdata, tapply(SPECIESRICHNESS, list(SITE, DIVER), mean))) ## total number of species per diver at each site
thenas<-data.frame(is.na(msd)) ## id all the locations of nas in dataset
toplot<-list()
...
species <- ggplot(toplot, aes(factor(diver), diver_diff))
species <- species + 
  geom_boxplot() + 
  coord_flip() + 
  geom_jitter(colour = "red", size =1, alpha = 0.4) + 
  theme_bw() + scale_y_continuous(limits=c(-AXIS_RANGE,AXIS_RANGE)) +
...
msd<-data.frame(with(compdata, tapply(HARD_CORAL, list(SITE, DIVER), mean))) ## mean biomass estimate per diver per site
thenas<-data.frame(is.na(msd)) ## id all the locations of nas in dataset
toplot<-list()
...
coral <- ggplot(toplot, aes(factor(diver), diver_diff))
coral <- coral + 
  geom_boxplot() + 
  coord_flip() + 
  geom_jitter(colour = "red", size =1, alpha = 0.4) + 
...
table(wd$SPECIES)
SPECIES_OF_INTEREST<-c("ACNF", "ACOL", "ZEFL", "CTST", "NAUN", "NALI", "NAUN", "APVI", "MENI", "BOBI", "CAAB", "CHMU", "MOGR", "PAMU", "CHSO", "CHPE", "SCRU", "THDU", "SUBU", "PAAR", "CAGA")
