...
df<-read.csv("fish_rea_base_kaylyn.csv")
...
#check data frame for column names that are missing
setdiff(DATA_COLS,colnames(df))
#add REGION to df
df$REGION<-df$REGION_NAME
df$OTHERBENTHIC<-df$OTHER
...
# SAVE FILE
getwd() # make sure it is in right folder
...
# WITHOUT  RAYS
wd<-wd[!wd$SPECIES %in% c("MABI", "AENA", "TAME", "PASE","HIFA"),]
